...
import type { CompiledReferenceNode, EngineValue, Sigma } from "../../../dist/esm/index.js";
...
function viewReferenceEnum(
  ...
  const nodeLabel = label ?? node.meta?.label ?? node.target;
...
export function viewReference(
  ...
  if (node.format?.enum) {
    return viewReferenceEnum(node, value, projStr, valStr, judgment, sigma, ctx, label);
  }
  return viewReferenceText(node, value, projStr, valStr, judgment, sigma, ctx, label);
}
